...
def embedding_node(state: AgentState):
    """Embed the user message and store it."""
    ...
    return {
        ...
        "last_embedding": embedding_array,
        "retrieved_memory": None
    }


def retrieve_node(state: AgentState):
    """Search DB using the embedding from embedding_node."""
    query_embedding = state["last_embedding"]

    if db.count() == 0:
        memory_context = "No prior memory available."
    else:
        retrieved = db.search_similar(query_embedding, top_k=5)
...
graph.add_edge("embedding_node", "retrieve_node")
